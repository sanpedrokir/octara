...
export async function GET() {
  try {
    ...
    const sql = db();
    const [row] = await sql`
      SELECT ca.*, i.name as industry_name, jr.name as job_role_name
...
export async function POST(request: Request) {
  try {
    const session = await requireAuth();
    const { industry_id, job_role_id, notes } = await request.json();
    if (!industry_id || !job_role_id) {
      return Response.json({ data: null, error: 'Industry and job role are required' }, { status: 400 });
    }

    const sql = db();
    const [row] = await sql`
      INSERT INTO career_aspirations (user_id, industry_id, job_role_id, notes, updated_at)
      VALUES (${session.userId}, ${industry_id}, ${job_role_id}, ${notes || null}, NOW())
